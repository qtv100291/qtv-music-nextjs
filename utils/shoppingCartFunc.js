...
import updateUser, { updateShoppingCart } from "../services/updateService";

function loadCartLocal() {
  //load infomation from qtv cart in localstorage
...
function saveCartLocal(shoppingCart) {
  //save infomation to qtv cart in localstorage
...
function merge2shoppingCart(cart_1, cart_2) {
  ...
  const itemOnlyInCart_1 = cart_1.filter(
    (item) => !commonItemIndex.includes(item.id)
  );
  const itemOnlyInCart_2 = cart_2.filter(
    (item) => !commonItemIndex.includes(item.id)
  );
  const shoppingCart = [
    ...itemOnlyInCart_1,
...
async function saveShoppingCart() {
  ...
  if (!isLogged) {
    ...
    return true
  } else return await updateShoppingCart();
}
